...
library("optparse");library("pophelper");library("ggplot2")
}

option_list = list(
  make_option(c("-p", "--prefix"), type="character", default="usnps_admixture",
              help="prefix name (with path if not in the current directory)", metavar="character"),
  make_option(c("-i", "--infofile"), type="character", default="usnps_admixture.txt",
              help="info text file containing for each individual the population/species information", metavar="character"),
  make_option(c("-k", "--maxK"), type="integer", default=5,
              help="maximum K value", metavar="integer"),
  make_option(c("-m", "--minK"), type="integer", default=2,
              help="minimum K value", metavar="integer"),
  make_option(c("-l", "--populations"), type="character", default="african_leopard,arabian_leopard,asian_leopard",
              help="comma-separated list of populations/species in the order to be plotted", metavar="character"),
  make_option(c("-o", "--outPrefix"), type="character", default="plot_final",
              help="output prefix (default: name provided with prefix)", metavar="character"),
  make_option(c("-t", "--treeOrder"), type="character", default="tree_order.txt",
              help="Info text with three columns, 1) species_name; 2)Population assignment; 3)Order Number", metavar="character")
)
...
# If no order given, use the population assignment
if(is.null(opt$treeOrder)) opt$treeOrder=opt$populations
...
ordering <- read.table(opt$treeOrder)

# Name the columns
names(labels)<-c("ind","pop")
names(ordering)<-c('ind', 'pop', 'order')
...
labels$order<-as.integer(ordering$order)
...
sfiles <- list.files(path = ".",pattern="*Q")
slist <- readQ(sfiles, filetype = "auto", indlabfromfile = T)
slistab <- tabulateQ(slist)
tbl <- alignK(slist)

#for (i in 1:length(tbl)) {
 # tbl[[i]] <- cbind(labels, tbl[[i]]) %>%
  #  sort(order)
#}
...
# Asign a color for each K
colorets = c("yellow4", "khaki1", "green3","darkorange2", "greenyellow", "sandybrown")

# Plot the cluster assignments as a single bar for each individual for each K as a separate row
pdf(file=paste0(opt$outPrefix,".pdf"),width = 11.7, height = 8.3)
 par(mfrow=c(maxK,1),mar=c(0,1,0,0),oma=c(3,2,12,2),mgp=c(0,0.7,0),xaxs="i",cex.lab=1,cex.axis=.8)
 # Plot minK
 # Use this color to specify colours
# col[[minK]]
# col[[x+1]]
 #E69F00
 asd = c("#EACE6A","light blue", "light green", "#D55E00", "red")
bp <- barplot(t(as.matrix(tbl[[1]][order(labels$order),])), col=asd ,xaxt="n", border=NA,ylab=paste0("K=",minK),yaxt="n",space=spaces)
 axis(3,at=bp,labels=labels$ind[order(labels$order)],las=2,tick=F,cex=0.5)
 
 # Plot higher K values
 if(maxK>minK)lapply(1:(maxK-1), function(x) barplot(t(as.matrix(tbl[[x+1]][order(labels$order),])), col=asd,xaxt="n", border=NA,ylab=paste0("K=",x+1),yaxt="n",space=spaces))
